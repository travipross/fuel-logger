...
class TestComputeStatsFromFillupDf:
    def test_with_sample(self, corolla_df):
    ...
    def test_with_fewer_than_2_rows(self, corolla_df):
        df_0_row = pd.DataFrame()
        df_1_row = corolla_df.iloc[:1]
        df_2_row = corolla_df.iloc[:2]
        df_3_row = corolla_df.iloc[:3]

        assert compute_stats_from_fillup_df(df_0_row) is None
        assert compute_stats_from_fillup_df(df_1_row) is None
        assert compute_stats_from_fillup_df(df_2_row) is not None
        assert compute_stats_from_fillup_df(df_3_row) is not None
